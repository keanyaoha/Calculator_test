        ...
        for activity in activities:
            ...
            try:
                ...
                if not factor_series.empty:
                    ...
                    if pd.isna(factor):
                         # st.warning(f"Missing emission factor for '{activity}' in {current_country}. Assuming 0 emission.") # Optional warning
                         st.session_state.emission_values[activity] = 0
                    else:
                         st.session_state.emission_values[activity] = user_input * float(factor)
                else:
                    # st.warning(f"Emission factor row not found for '{activity}' in {current_country}. Assuming 0 emission.") # Optional warning
                    st.session_state.emission_values[activity] = 0
            except Exception as e:
...
# This block is now empty after removing the st.info message
