...
st.markdown(r"""
    <style>
        ...
        /* This selector might need adjustment depending on Streamlit version */
        /* It targets the div containing the label text when the input is checked */
        div[role="radiogroup"] input[type="radio"]:checked + div {
            /* Overriding Streamlit's default active tab style may be complex */
            /* Keeping previous style for reference, might need inspection */
             background-color: white; /* Default Streamlit uses white for active tab background */
             border-bottom: 1px solid white;
             font-weight: bold;
             color: #1a9850; /* Your desired Green text color for selected tab */
        }
        ...
        /* This targets ALL buttons by default, might need refinement */
        /* Let's refine to target non-primary buttons specifically if possible */
        /* Assuming non-primary buttons don't have 'kind="primary"' */
        div[data-testid="stButton"] button:not([kind="primary"]) {
            background-color: #61c2a2;  /* Lighter Green background */
        ...
        /* --- Primary Calculate Button Styling (Keep this specific) --- */
        div[data-testid="stButton"] button[kind="primary"] {
            background-color: #1a9850; /* Darker Logo Green */
            color: white;
            border: none;
            padding: 0.5rem 1rem;
            border-radius: 0.25rem;
        ...
        div[data-testid="stButton"] button[kind="primary"]:hover {
            background-color: #147a40; /* Even darker green on hover */
            color: white;
            border: none;
        }

        /* Focus state for ALL buttons (consistency) */
        div[data-testid="stButton"] button:focus {
            outline: none;  /* Remove the default focus outline */
            box-shadow: 0 0 0 0.2rem rgba(26, 152, 80, 0.5); /* Green shadow on focus */
        }
    </style>
...
# Removed divider here, let spacing be handled by elements

# --- Country Selection ---
st.markdown("**Select your country**") # Step numbering removed for simplicity
...
if st.session_state.selected_country != "-- Select --":
    ...
    st.markdown("**Enter your monthly consumption details**") # Step numbering removed

    # Add emojis to tab labels
    ...
    def display_activity_inputs(activities, category_key, current_country):
        ...
        for activity in activities:
            ...
            # Store user input back to state
            st.session_state.emission_values[f"{input_key}_input"] = user_input
            # Calculate and store emission value
            try:
                # Check if activity exists in the loaded dataframe's 'Activity' column
                if activity not in df['Activity'].values:
                     st.session_state.emission_values[activity] = 0.0
                     continue # Skip calculation if activity is not valid

                factor_series = df.loc[df["Activity"] == activity, current_country]

                if not factor_series.empty:
                    factor = factor_series.iloc[0]
                    # Handle potential NaN factors
                    if pd.isna(factor):
                        st.session_state.emission_values[activity] = 0.0
                    else:
                        st.session_state.emission_values[activity] = user_input * float(factor)
                else:
                    # Factor row not found (should be rare if activity check passed)
                    st.session_state.emission_values[activity] = 0.0
            except KeyError:
                 # This indicates the country column doesn't exist - should be caught by available_countries
                 # st.error(f"Data error: Country '{current_country}' column missing.") # Optional user error
                 st.session_state.emission_values[activity] = 0.0
            except Exception as e:
                # Generic error catch during calculation
                # st.error(f"Calculation error for {label}: {e}") # Optional user error
                st.session_state.emission_values[activity] = 0.0

    # Define Activity Lists (keep these consistent)
    ...
    # Use columns consistently for button placement if desired
    col_btn_left, col_btn_right = st.columns([1,1]) # Example: Even split

    if current_index == 0: # Transport
        display_activity_inputs(transport_activities, "transport", country)
        # Place button using columns if needed, or directly if only one button
        # Aligning single button right might need specific column setup or CSS
        button_placeholder = st.empty() # Use placeholder if alignment is tricky
        with col_btn_right: # Try placing in right column
             if st.button("Next →", key="next_transport", use_container_width=True): # Use container width for full column width
        ...
        if reviewed_all:
            ...
            if st.button("Calculate My Carbon Footprint", type="primary", use_container_width=True, key="calculate_final_button"):
                ...
                if not emission_values_to_sum:
                    ...
                    # Store comparison data for results display
            ...
            # Prompt user if checkbox is not ticked
            st.info("Check the box above to enable calculation.")

    # --- Display Results Area (Conditionally Shown Below Tabs) ---
    if st.session_state.get('calculation_done', False):
        ...
        if total_emission > 0:
            ...
            # Calculate and display tree equivalence
            tree_absorb_monthly = (21.77 / 12.0) # kg CO2 / tree / month
            ...
            plot_data_list = []
            captions = []
            world_avg_value = None

            # Safely get world average for comparison
            if comparison_data and isinstance(comparison_data.get('world'), dict):
                world_avg_value = comparison_data['world'].get('avg')

            # --- Determine 'You' bar color ---
            you_color = '#1a9850' # Default: Logo Green
            color_reason = ""
            if world_avg_value is not None:
                if total_emission > world_avg_value:
                    you_color = '#e41a1c' # Red for above average
                    color_reason = "(above world average)"
                else:
                     color_reason = "(below or equal to world average)"
            color_map = {'You': you_color, 'Average': '#a6cee3'} # Consistent light blue for averages

            # --- Prepare data list for DataFrame ---
            plot_data_list.append({"Source": "You", "Emissions": total_emission, "Type": "You"})

            if comparison_data:
                # Loop through averages and add if available
                for key, type_label, default_name in [("country", "Average", country), ("eu", "Average", "EU Average"), ("world", "Average", "World Average")]:
                    ...
                    if avg is not None:
                         if name != "You": # Prevent duplication if country name is 'You'
                            plot_data_list.append({"Source": name, "Emissions": avg, "Type": type_label})
                    elif key in comparison_data: # Only note missing if expected
                        captions.append(f"Note: Average data for {name} not available.")

            # Display captions about missing data
            ...
            if plot_data_list:
                ...
                sustainable_target = 167 # kg CO2e/month target
                ...
                try:
                    ...
                    # Add target line
                    fig_comp.add_vline(
                        x=sustainable_target, line_width=2, line_dash="dash", line_color="grey",
                        annotation_text=f"~{sustainable_target} kg: Long-term Target*",
                        annotation_position="top right", annotation_font_size=10, annotation_font_color="grey"
                    )
                    ...
                    fig_comp.update_layout(
                         ...
                         bargap=0.4, # Adjust bar thickness via gap
                         margin=dict(l=5, r=5, t=30, b=20), # Margins
                         showlegend=False,
                         title_text="Monthly Carbon Footprint Comparison",
                         title_x=0.5, # Center title
                         title_font_size=16,
                         # Ensure backgrounds are transparent if desired
                    ...
                    # st.error(traceback.format_exc()) # Uncomment for full traceback during debug

            else: # Should not happen if total_emission > 0
                st.warning("No data available for comparison plot.")
        else:
            # Handle case where calculation resulted in zero emissions
            st.info("Your calculated emissions are zero.")

# --- Prompt if no country selected ---
elif not st.session_state.selected_country or st.session_state.selected_country == "-- Select --":
    st.info("Please select your country to begin.")

# --- Sidebar ---
st.sidebar.markdown("---")
# Add other potential sidebar content if needed
